<div id="fixtures-table" class="football-table-wrap" data-season="<?php echo $season; ?>" data-league_id="<?php echo $league_id; ?>">
    <div class="standing-header"><h3>Fixtures</h3></div>
    <?php
        foreach($fixtures_data as $timestamp => $rounds) {
            krsort($rounds);
    ...
    <?php if( $atts['page_id'] != '' ): ?>
        <div class="football-table-link table-container-row">
            <div class="table-container-cell table-container-cell--wide" colspan="3"><a href="<?php echo get_the_permalink($atts['page_id']); ?>" class="view-full-table">View full table</a></div>
        </div>
    <?php endif; ?>
</div>